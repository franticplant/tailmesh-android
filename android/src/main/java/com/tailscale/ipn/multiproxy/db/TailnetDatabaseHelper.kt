...
class TailnetDatabaseHelper(context: Context) :
  ...
  override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
    if (oldVersion < 2) {
      ...
      try {
        ...
        db.execSQL(
            "ALTER TABLE $TABLE_PROFILES ADD COLUMN $COL_STANDARD_SELECTED INTEGER NOT NULL DEFAULT 0")
        db.execSQL(
            "ALTER TABLE $TABLE_PROFILES ADD COLUMN $COL_MIGRATION_VERSION INTEGER NOT NULL DEFAULT 1")
        db.execSQL(
            "CREATE UNIQUE INDEX IF NOT EXISTS profiles_source_profile_id ON $TABLE_PROFILES($COL_SOURCE_PROFILE_ID)")
    ...
    if (oldVersion < 4) {
      ...
      try {
        db.execSQL(
            "ALTER TABLE $TABLE_UPSTREAMS ADD COLUMN $COL_UPSTREAM_SOURCE_TAILNET TEXT NOT NULL DEFAULT ''")
        db.execSQL(
            "ALTER TABLE $TABLE_UPSTREAMS ADD COLUMN $COL_UPSTREAM_PEER_ADDR TEXT NOT NULL DEFAULT ''")
        db.setTransactionSuccessful()
      } finally {
        db.endTransaction()
      }
    ...
    if (oldVersion < 5) {
      ...
      try {
        db.execSQL(
            "ALTER TABLE $TABLE_APP_BINDINGS ADD COLUMN $COL_BINDING_DNS_UPSTREAM TEXT NOT NULL DEFAULT ''")
        db.setTransactionSuccessful()
      } finally {
        db.endTransaction()
      }
